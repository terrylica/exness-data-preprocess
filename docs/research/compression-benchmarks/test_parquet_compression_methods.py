...
def test_compression_methods():
    ...
    for level in zstd_levels:
        try:
            ...
            pq.write_table(table, parquet_path, compression="zstd", compression_level=level)
    ...
    for level in brotli_levels:
        try:
            ...
            table = pa.Table.from_pandas(df)
            pq.write_table(table, parquet_path, compression="brotli", compression_level=level)
    ...
    for _, row in df_results.iterrows():
        ...
        print(
            f"{row['method']:<15} {row['level']:<10} {row['size_mb']:<12.2f} {row['vs_zip']:<10.2f} {savings_str:<10}"
        )
    ...
    print(
        f"Parquet ({best['method']}, level {best['level']}):  {parquet_total:>8.1f} MB ({parquet_total / 1024:.2f} GB)"
    )
    print(f"                             {'─' * 30}")
    ...
    if parquet_total < zip_total:
        ...
        print(
            f"Overhead:                    {overhead:>8.1f} MB ({100 * overhead / zip_total:.1f}%)"
        )
    ...
    if parquet_total < zip_total:
        ...
        print("✓ Delete ZIPs after conversion")
        print(f"✓ Save {(zip_total - parquet_total) / 1024:.2f} GB for 3 years")
    else:
        print("\n✗ Keep Option A (DuckDB only, 28 MB)")
        print("✗ Don't store Parquet permanently (larger than ZIPs)")
        print("✓ Use on-demand tick analysis (download → analyze → delete)")
