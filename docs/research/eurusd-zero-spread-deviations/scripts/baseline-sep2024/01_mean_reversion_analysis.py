...
print(f"   ├─ Bid-biased (N={len(deviation_df[deviation_df['deviation_type']=='bid_biased']):,}): Mean position={deviation_df[deviation_df['deviation_type']=='bid_biased']['position_ratio'].mean():.3f}")
print(f"   └─ Ask-biased (N={len(deviation_df[deviation_df['deviation_type']=='ask_biased']):,}): Mean position={deviation_df[deviation_df['deviation_type']=='ask_biased']['position_ratio'].mean():.3f}")
...
for horizon_sec in REVERSION_HORIZONS:
    ...
    print(f"   └─ Mean reversion amount: {mean_reversion_amount:+.4f} {'✅ REVERSION' if mean_reversion_amount > 0 else '❌ DIVERGENCE'}")

    print(f"\n   By Deviation Type:")
    if len(bid_sample) > 10:
        bid_reverted = bid_sample['reverted_to_mid'].mean() * 100
        bid_toward = bid_sample['moved_toward_mid'].mean() * 100
        print(f"   ├─ Bid-biased (N={len(bid_sample):,}): {bid_reverted:.1f}% reverted, {bid_toward:.1f}% moved toward")

    if len(ask_sample) > 10:
        ask_reverted = ask_sample['reverted_to_mid'].mean() * 100
        ask_toward = ask_sample['moved_toward_mid'].mean() * 100
        print(f"   └─ Ask-biased (N={len(ask_sample):,}): {ask_reverted:.1f}% reverted, {ask_toward:.1f}% moved toward")
...
print(f"✅ Saved: /tmp/mean_reversion_report.md")
