...
print("   ✅ Feature 1: Deviation magnitude")

# Feature 2: Deviation persistence (duration)
print("   Computing deviation persistence...")
...
deviation_df["is_consecutive"] = (deviation_df["time_to_next"].shift(1) <= PERSISTENCE_WINDOW) & (
...
print(
    f"   ✅ Feature 2: Deviation persistence (mean={deviation_df['feat_persistence_sec'].mean():.1f}s)"
)

# Feature 3: Spread width at deviation time
deviation_df["feat_spread_width_bps"] = deviation_df["std_spread"] * 10000
print(
    f"   ✅ Feature 3: Spread width (mean={deviation_df['feat_spread_width_bps'].mean():.2f} bps)"
)

# Feature 4: Recent volatility (GARCH-like)
print("   Computing recent volatility...")
...
print(
    f"   ✅ Feature 4: Recent volatility (mean={deviation_df_sample['feat_recent_volatility_bps'].mean():.2f} bps)"
)
...
deviation_df_sample = deviation_df_sample.dropna(
    subset=[f"target_volatility_{FUTURE_HORIZONS[0]}m"]
)
...
print("✅ Saved: /tmp/enhanced_volatility_model_report.md")
...
print("\n📊 Key Takeaways:")
print(
    f"   Best R²: {results_df['r_squared'].max():.4f} ({results_df.loc[results_df['r_squared'].idxmax(), 'horizon_min']:.0f}-min horizon)"
)
