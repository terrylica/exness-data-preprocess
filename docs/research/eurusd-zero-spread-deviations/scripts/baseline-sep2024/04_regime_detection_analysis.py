    ...
    if row['is_clustered'] and len(current_cluster) > 0:
        ...
        if len(current_cluster) >= MIN_CLUSTER_SIZE:
            ...
            clusters.append({
                ...
                'dominant_type': max(set([d['deviation_type'] for d in current_cluster]),
...
clusters_sample['volatility_shift'] = clusters_sample['after_volatility'] - clusters_sample['before_volatility']
clusters_sample['volatility_shift_pct'] = (clusters_sample['volatility_shift'] / clusters_sample['before_volatility'] * 100)
clusters_sample['trend_shift'] = clusters_sample['after_trend'] - clusters_sample['before_trend']
clusters_sample['spread_shift'] = clusters_sample['after_mean_spread'] - clusters_sample['before_mean_spread']
...
print("-" * 80)
print(f"   Increased volatility: {volatility_increased:,} ({volatility_increased/len(clusters_sample)*100:.1f}%)")
print(f"   Decreased volatility: {volatility_decreased:,} ({volatility_decreased/len(clusters_sample)*100:.1f}%)")
print(f"   Significant increase (>20%): {significant_vol_increase:,} ({significant_vol_increase/len(clusters_sample)*100:.1f}%)")
print(f"   Significant decrease (<-20%): {significant_vol_decrease:,} ({significant_vol_decrease/len(clusters_sample)*100:.1f}%)")

print(f"\n   Mean changes:")
print(f"   ├─ Volatility: {clusters_sample['volatility_shift'].mean():+.4f} bps ({clusters_sample['volatility_shift_pct'].mean():+.1f}%)")
...
for cluster_type in ['bid_biased', 'ask_biased']:
    ...
    print(f"   ├─ Sig vol increases: {vol_sig_increase:,} ({vol_sig_increase/len(type_clusters)*100:.1f}%)")
    print(f"   ├─ Mean vol shift: {type_clusters['volatility_shift'].mean():+.4f} bps ({type_clusters['volatility_shift_pct'].mean():+.1f}%)")
...
size_bins = [clusters_sample['size'].min(), size_quartiles[0], size_quartiles[1], size_quartiles[2], clusters_sample['size'].max()]
size_labels = ['Small', 'Medium', 'Large', 'XLarge']

clusters_sample['size_category'] = pd.cut(clusters_sample['size'], bins=size_bins, labels=size_labels, include_lowest=True)
...
for size_cat in size_labels:
    ...
    print(f"   ├─ Sig vol increases: {vol_sig_increase:,} ({vol_sig_increase/len(size_clusters)*100:.1f}%)")
...
print(f"✅ Saved: /tmp/regime_detection_clusters.csv")
...
print(f"✅ Saved: /tmp/regime_detection_report.md")
...
print(f"   Predictive power: p={p_value:.4f} {'✅ SIGNIFICANT' if p_value < 0.05 else '❌ NOT SIGNIFICANT'}")
print(f"   Vol increase rate: {observed_vol_increase_rate*100:.1f}% (baseline {baseline_vol_increase_rate*100:.0f}%)")
print(f"   Effect size: {effect_size*100:+.1f} percentage points")
