...
print("\n📊 Configuration:")
print(
    f"   Extreme thresholds: <{EXTREME_THRESHOLDS['extreme_bid']} or >{EXTREME_THRESHOLDS['extreme_ask']}"
)
print(
    f"   Regular thresholds: <{REGULAR_THRESHOLDS['bid_biased']} or >{REGULAR_THRESHOLDS['ask_biased']}"
)
...
merged_df.loc[
    merged_df["position_ratio"] < EXTREME_THRESHOLDS["extreme_bid"], "deviation_category"
] = "extreme_bid"
merged_df.loc[
...
merged_df.loc[
    merged_df["position_ratio"] > EXTREME_THRESHOLDS["extreme_ask"], "deviation_category"
] = "extreme_ask"
...
print(
    f"   ├─ Normal (midpoint ±0.1): {normal_count:,} ({normal_count / len(merged_df) * 100:.1f}%)"
)
print(
    f"   ├─ Regular bid (0.2-0.4): {regular_bid_count:,} ({regular_bid_count / len(merged_df) * 100:.1f}%)"
)
print(
    f"   ├─ EXTREME bid (<0.2): {extreme_bid_count:,} ({extreme_bid_count / len(merged_df) * 100:.1f}%)"
)
print(
    f"   ├─ Regular ask (0.6-0.8): {regular_ask_count:,} ({regular_ask_count / len(merged_df) * 100:.1f}%)"
)
print(
    f"   └─ EXTREME ask (>0.8): {extreme_ask_count:,} ({extreme_ask_count / len(merged_df) * 100:.1f}%)"
)
...
def detect_crisis_events(ts, std_indexed, horizons):
    ...
    """
    current_price = (
        std_indexed.loc[std_indexed.index <= ts].iloc[-1]["mid"]
        if len(std_indexed.loc[std_indexed.index <= ts]) > 0
        else np.nan
    )

    if pd.isna(current_price):
        return (
            {f"{h}s_flash_crash": np.nan for h in horizons}
    ...
    for horizon_sec in horizons:
        ...
        max_move = (
            max(abs(max_price - current_price), abs(min_price - current_price))
            / current_price
            * 10000
        )
        ...
        metrics[f"{horizon_sec}s_extreme_vol"] = (
            volatility >= EXTREME_VOLATILITY_THRESHOLD if not pd.isna(volatility) else False
        )
...
print("   Analyzing extreme deviations...")
...
print("   Analyzing normal positions (control group)...")
...
print("✅ Saved: /tmp/liquidity_crisis_report.md")
...
print(
    f"   Statistical significance: p={p_value:.4f} {'✅ SIG' if p_value < 0.05 else '❌ NOT SIG'}"
)
print(
    f"   Conclusion: Extreme deviations {'ARE' if avg_flash_lift > 0 and p_value < 0.05 else 'are NOT'} crisis predictors"
)
