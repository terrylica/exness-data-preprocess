...
def load_month_data(year: str, month: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    merged = pd.merge_asof(
        rs_df, std_df, on="Timestamp", direction="backward", tolerance=pd.Timedelta(seconds=10)
    )
...
def calculate_rolling_risk_levels(deviations: pd.DataFrame) -> pd.DataFrame:
    ...
    total_hours = (
        deviations["Timestamp"].max() - deviations["Timestamp"].min()
    ).total_seconds() / 3600
...
def prepare_visualization_data(year: str, month: str) -> Path:
    ...
    pd.DataFrame(trading_zones_month).to_parquet(
        OUTPUT_DIR / f"viz_{month_str}_trading_zones.parquet"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Prepare visualization data for zero-spread deviations"
    )
