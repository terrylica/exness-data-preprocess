...
def detect_bursts(deviations: pd.DataFrame) -> dict:
    ...
    if len(burst_events) > 0 and len(quiet_events) > 0:
        ...
        burst_persistence = (
            p_burst_given_burst / p_burst_given_quiet if p_burst_given_quiet > 0 else np.nan
        )
    ...
    return {
        ...
        "p_burst_given_burst": float(p_burst_given_burst)
        if not pd.isna(p_burst_given_burst)
        else 0.0,
        "p_burst_given_quiet": float(p_burst_given_quiet)
        if not pd.isna(p_burst_given_quiet)
        else 0.0,
...
def analyze_month(year: str, month: str, hot_zones_all: pd.DataFrame) -> tuple:
    ...
    logger.info(
        f"  Bursts: {burst['burst_rate']:.1%} of events, "
        f"persistence={burst['burst_persistence']:.2f}×"
    )

    if burst["bursts_predict_bursts"]:
        logger.info("    ✅ Bursts predict bursts (persistence > 2×)")
    ...
    for zone in trading_zones:
        logger.info(
            f"  Zone {zone['price_level']:.4f}: {zone['risk_level']} "
...
def main():
    ...
    if all_conditional:
        ...
        avg_ratio = np.mean(
            [c["interval_ratio"] for c in all_conditional if not pd.isna(c["interval_ratio"])]
        )
    ...
    if all_burst:
        ...
        avg_persistence = np.mean(
            [b["burst_persistence"] for b in all_burst if not pd.isna(b["burst_persistence"])]
        )
